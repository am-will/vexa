...
These events are also published on the Redis channel `va:meeting:{meeting_id}:events`. See the [Voice Agent guide](voice-agent) for full details on controlling the bot's voice, chat, and screen share capabilities.
