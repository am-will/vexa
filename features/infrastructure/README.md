...
**Confidence: 75%** (gate: 100%, status: ❌ below gate)
...
| redis-up | Redis responds to PING | 10 | ❌ fail | `lite`: smoke-health/REDIS_UP: Redis responds to PING — WebSocket pub/sub, session state, and caching work; `compose`: smoke-health/REDIS_UP: Redis responds to PING — WebSocket pub/sub, session state, and caching work; `helm`: smoke-health/REDIS_UP: redis-cli ping:  |
| minio-up | MinIO is healthy / has ready replicas | 10 | ❌ fail | `compose`: smoke-health/MINIO_UP: MinIO responds — recordings and browser state storage work; `helm`: smoke-health/MINIO_UP: minio: 0 ready replicas |
