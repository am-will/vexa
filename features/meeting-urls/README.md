...
<!-- BEGIN AUTO-DOD -->
<!-- Auto-written by tests3/lib/aggregate.py from release tag `0.10.0-260430-1309`. Do not edit by hand — edit the sidecar `dods.yaml` + re-run `make -C tests3 report --write-features`. -->
...
| invalid-rejected | Invalid meeting URL returns 400 (not 500) | 10 | ❌ fail | `lite`: smoke-contract/INVALID_URL_REJECTED: HTTP 403 (expected one of [400, 422]); `compose`: smoke-contract/INVALID_URL_REJECTED: HTTP 403 (expected one of [400, 422]); `helm`: smoke-contract/INVALID_URL_REJECTED: HTTP 403 (expected one of [400, 422]) |
